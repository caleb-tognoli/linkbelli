<svelte:head><title>Edit template - linkbelli</title></svelte:head>
...
<script lang="ts">
	...
	$effect(() => {
		api.get(`/templates/${id}`).then(async (res) => {
			...
			fields = t.userFields.map((f: FieldForm) => ({ ...f, description: f.description ?? '' }));
			visibility = t.visibility;
			tags = t.tags ?? [];
			if (t.defaultSchedule && t.defaultSchedule !== CRON_NEVER) {
	...
	async function save() {
		...
		error = null;
		try {
			const res = await api.patch(`/templates/${id}`, {
				name,
	...
	<div class="mt-6 flex flex-col gap-5">
		...
		<div class="grid grid-cols-2 gap-4">
			...
			<div class="flex flex-col gap-1 text-sm">
				<span>Type</span>
				<span class="rounded-md border px-3 py-2 text-sm" style="border-color: var(--color-border); background: var(--color-surface)">
		...
		<div class="flex flex-col gap-2">
			...
			{#if fields.length === 0}
				<p class="text-sm" style="color: var(--color-muted)">No user fields yet.</p>
			{/if}
