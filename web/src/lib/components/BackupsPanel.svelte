<script lang="ts">
	...
	let {
		...
		enabled?: boolean;
		/** What the page already fetched, so the list is there in the first paint. */
		initial?: Backup[] | null;
	} = $props();

	let enabled = $state(initialEnabled);
	let backups = $state<Backup[]>(initial ?? []);
...
<div>
	...
	{#if loading}
		<SkeletonRows rows={3} class="mt-3" />
