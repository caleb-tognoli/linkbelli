...
/// <c>lbk_{publicId}_{secret}</c>: the publicId is hex (so it never collides with the
/// '_' delimiter) and stored for indexed lookup; the secret is high-entropy (256-bit)
/// so a plain SHA-256 is sufficient — no password-style slow hashing needed.
/// </summary>
public static class ApiKeyToken
{
    public const string HeaderName = "X-Api-Key";
    public const string TokenPrefix = "lbk";
    private const int PublicIdBytes = 9;  // -> 18 hex chars
