...
/// <summary>
/// A person's own playlists: making, reading, changing and removing them, taking a copy of
/// somebody else's, and how they like to look at each one.
/// </summary>
/// <remarks>
/// This used to be the whole of the playlist surface in one class of nearly twelve hundred lines —
/// the anonymous public reads, discovery and its rankings, likes and source attachment as well.
/// Three of the worst performance problems in the app were buried in it precisely because they
/// sat among five unrelated concerns. Those concerns each have their own service now:
/// <see cref="PublicPlaylistService"/>, <see cref="DiscoveryService"/>,
/// <see cref="PlaylistLikeService"/> and <see cref="PlaylistSourceService"/>.
/// </remarks>
    ...
    IAuditLog audit) : IPlaylistService
{
        ...
        Guid ownerId, int? limit, string? cursor, string[]? tags, string? q = null, bool unfiled = false, CancellationToken ct = default)
    {
        ...
        var query = db.Playlists.Where(p => p.OwnerId == ownerId).WithEveryTag(tags);
    ...
    public Task<IReadOnlyList<TagSummary>> ListOwnTagsAsync(Guid ownerId, string? q, CancellationToken ct = default) =>
        TagCounts.ListAsync(db.PlaylistTags.Where(pt => pt.Playlist!.OwnerId == ownerId), q, ct);
